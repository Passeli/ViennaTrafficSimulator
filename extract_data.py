...
# ==========================================
print("Loading Vienna Polygon and downloading street network...")
apocalypse_filter = (
    '["area"!~"yes"]'
...
print("Cleaning speeds and calculating travel times for fastest routing...")

for u, v, key, data in G.edges(keys=True, data=True):
    max_speed_kmh = data.get('maxspeed', 50.)

    if isinstance(max_speed_kmh, list):
        max_speed_kmh = max(map(float, max_speed_kmh))
    elif isinstance(max_speed_kmh, str):
        max_speed_kmh = 10. if max_speed_kmh.lower() == 'walk' else float(max_speed_kmh)
...
# ==========================================
print("Calculating fastest Dijkstra routes to exit nodes...")
exit_nodes = [
    # right bank
...
for node, path in paths.items():
    if len(path) > 1:
        G.nodes[node]['next_node_to_exit'] = path[-2]
    elif len(path) == 1:
        G.nodes[node]['next_node_to_exit'] = -1
    else:
...
# ==========================================
print("Fetching parking and building features...")
...
valid_parking['capacity'] = 0.

geom_types = valid_parking.geometry.geom_type
area = valid_parking.geometry.area
lengths = valid_parking.geometry.length

if 'amenity' not in valid_parking.columns:
    valid_parking['amenity'] = ''
if 'building' not in valid_parking.columns:
    valid_parking['building'] = ''
if 'parking' not in valid_parking.columns:
    valid_parking['parking'] = ''
...
# all houses / apartments / residential -> area / 50
valid_parking.loc[is_poly & is_residential, 'capacity'] = area / 50.

# surface parking lots & 1-story garages -> area / 25
valid_parking.loc[is_poly & (is_surface_lot | is_garages_row), 'capacity'] = area / 25.

# multi-story & underground parking decks -> area / 5
valid_parking.loc[is_poly & (is_parking_deck | is_underground_or_multi), 'capacity'] = area / 5.

# single garage & single parking space -> 1 car
valid_parking.loc[is_poly & (is_garage_single | is_parking_space), 'capacity'] = 1.

is_line = geom_types.isin(['LineString', 'MultiLineString'])

# any parking mapped as a line -> 1 car per 5 meters
valid_parking.loc[is_line & (is_parking_lot | is_parking_space), 'capacity'] = lengths / 5.

is_point = geom_types.isin(['Point', 'MultiPoint'])

# a point tagged simply as 'parking' -> 10 cars
valid_parking.loc[is_point & is_parking_lot, 'capacity'] = 10.

# a point tagged as a single space or single garage -> 1 car
valid_parking.loc[is_point & (is_parking_space | is_garage_single), 'capacity'] = 1.
...
print("Compiling 0-indexed memory maps...")
...
print("Baking Dijkstra routes directly into Edge memory...")
...
print("Packing flat binary files for Vulkan RWStructuredBuffers...")
...
with open("vulkan_nodes.bin", "wb") as f:
    for node_id in G.nodes():
        ...
        f.write(struct.pack('ffii', x, y, lock, node_type))  # 16 bytes
...
with open("vulkan_edges.bin", "wb") as f:
    for i in range(num_edges):
        ...
        f.write(struct.pack('iiiffiii',
                            ...
                            float(length), max_speed_ms, -1, -1, spawn_cap))  # 32 bytes
